...
def _select_audio_validation_indices(val_ds, samples_per_dialect: int):
    if len(val_ds) == 0 or samples_per_dialect <= 0:
        return []

    if "dialect" not in val_ds.column_names:
        return list(range(min(samples_per_dialect, len(val_ds))))
...
def validate_audio(
    ...
    selected_indices = _select_audio_validation_indices(val_ds, samples_per_dialect)
